...
//  (cada página carrega o script de novo, mas a posição/estado ficam em
//  localStorage — pedido do Luan, 18/08/2026: "quando sair dessa página,
//  a barra de progresso continua na tela").
// ─────────────────────────────────────────────
(function () {
  ...
  const css = ''
    + '.jw-widget{position:fixed;right:20px;bottom:20px;width:280px;background:var(--surface);'
    ...
    + '.jw-widget.jw-collapsed{width:auto}'
    ...
    + '.jw-body{max-height:280px;overflow-y:auto;padding:4px}'
    + '.jw-job{padding:8px 10px;border-radius:8px}'
    + '.jw-job+.jw-job{margin-top:2px}'
    + '.jw-job-top{display:flex;align-items:baseline;justify-content:space-between;gap:8px}'
    + '.jw-job-label{font-size:11px;font-weight:600;color:var(--text)}'
    ...
    + '@media(max-width:768px){.jw-widget{right:12px;bottom:12px;left:12px;width:auto}}';
  ...
  function mount() {
    ...
    // Posição arrastada persiste entre páginas (localStorage) — sem posição salva, fica no
    // canto inferior direito (padrão via CSS).
    try {
  ...
  function jobRowHtml(job) {
    const statusClass = job.status === 'done' ? 'jw-job-done' : job.status === 'error' ? 'jw-job-error' : '';
    const barClass = job.progressPct == null ? 'jw-bar jw-bar-indeterminate' : 'jw-bar';
    const barWidth = job.progressPct == null ? '' : ('width:' + job.progressPct + '%;');
    const by = job.startedBy ? ('por ' + esc(job.startedBy)) : 'automático';
    const msg = job.status === 'done' ? 'Concluído' + (job.message ? ': ' + job.message : '')
      : job.status === 'error' ? 'Erro: ' + (job.message || '')
    ...
    return '<div class="jw-job ' + statusClass + '">'
      ...
      + '<span class="jw-job-by">' + by + '</span></div>'
